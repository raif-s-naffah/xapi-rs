...
/// A doppelgänger structure, similar to that of [StatementId], that abides
/// by the 'ids' format rules, but this time for [StatementResult].
/// 
#[skip_serializing_none]
...
impl StatementResultId {
    ...
    /// Return whether this is empty (TRUE); i.e. has zero [StatementId]s,
    /// or not (FALSE).
    pub fn is_empty(&self) -> bool {
        self.statements.is_empty()
    }
