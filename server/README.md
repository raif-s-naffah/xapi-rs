...
In earlier versions, there were 3 main modules in this project that cover:

1. `data` &ndash; the data structures involved. This is now its own crate `xapi-data`.
...
the first is now a separate crate w/ its own version, published as `xapi-data`.  Separating the rest is still a work-in-progress. For now _LaRS_ (the LRS server proper) is effectively the `server` member of the Workspace published (still) as `xapi-rs`.

Changes are tracked in [ChangeLog](CHANGELOG.md) for each of those crates.
...
2. **PostgreSQL**
    ...
    As mentioned in the documentation, this project relies on an installed **PostgreSQL** database system. FWIW, as of the last edit date of this document, the version of **PostgreSQL** was 18.3.
