...
**IMPORTANT** - **`LaRS`** allows using the last segment of certain [Vocabulary verbs][3] as an alias. For those _Verbs_ using that short _alias_ instead of the full IRI is allowed in this context. For example using `answered` instead of `http://adlnet.gov/expapi/verbs/answered` is acceptable.
